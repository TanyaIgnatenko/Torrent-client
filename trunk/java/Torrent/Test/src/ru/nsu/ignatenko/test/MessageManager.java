...
public class MessageManager
{
    ...
    public void sendHandshake(OutputStream output) throws IOException
    {
        ...
        output.write(handshake.getProtocolName(), 0, protocolNameLength);
        ...
        output.write(handshake.getInfoHash(), 0, HASH_LENGTH);
        output.write(handshake.getPeerID(), 0, PEER_ID_LENGTH);
    }
    ...
    public Handshake receiveHandshake(InputStream input) throws IOException
    {
        ...
        {
            throw new IOException("Invalid clientHandshake1.");
        }
        ...
        {
            throw new IOException("Invalid clientHandshake2.");
        }

        bytesRead = input.read(buf, 0, HASH_LENGTH);
        if (bytesRead != HASH_LENGTH)
        {
            throw new IOException("Invalid clientHandshake3.");
        }
        ...
        {
            throw new IOException("Invalid clientHandshake4.");
        }
